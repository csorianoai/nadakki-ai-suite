...
def analyze_agent_file(file_path):
    ...
    if "has_execute_method" in categories and results["lines"] > 50:
        if score >= 60:
            ...
            results["recommendation"] = "⚠️  Necesita validación"
        else:
            results["final_category"] = "AGENTE_BASICO"
            results["recommendation"] = "⚠️  Requiere desarrollo"
        ...
        if results["lines"] > 30:
            ...
            results["recommendation"] = "🗑️  Considerar eliminación"
    
    elif results["lines"] > 10:
        results["final_category"] = "UTILIDAD_SCRIPT"
        results["recommendation"] = "🛠️  Mantener como utilidad"
    
    else:
        results["final_category"] = "ARCHIVO_MINIMO"
        results["recommendation"] = "🗑️  Eliminar si no es necesario"
    ...
    if placeholder_count > 3 and results["lines"] < 30:
        ...
        results["recommendation"] = "🗑️  Eliminar (placeholder sin implementar)"
...
def main():
    ...
    for i, file_path in enumerate(python_files, 1):
        ...
        print(f"  [{i}/{len(python_files)}] Analizando: {file_path.relative_to(agents_root)}")
        analysis = analyze_agent_file(file_path)
        
        # Agregar información adicional
        analysis["relative_path"] = str(file_path.relative_to(agents_root))
    ...
    print(f"   ✅ MANTENER: {keep_count} archivos")
    print(f"   ⚠️  REVISAR: {review_count} archivos")
    print(f"   🗑️  ELIMINAR: {delete_count} archivos")
        ...
        for category in report["recommendations"]["delete"]:
            if category in summary_stats:
                ...
                if len(summary_stats[category]["files"]) > 10:
                    print(f"   ... y {len(summary_stats[category]['files']) - 10} más")
    ...
    if review_count > 0:
        ...
        for category in report["recommendations"]["review"]:
            if category in summary_stats:
                print(f"   ⚠️  {category}: {summary_stats[category]['count']} archivos")
