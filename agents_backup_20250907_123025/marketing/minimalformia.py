"""
Minimalformia - Marketing Ecosystem
Optimiza formularios de precalificación con IA
Generado automáticamente: 2025-08-07T13:29:04.328289
Performance: 94.7%
Complejidad: medium
"""
...
class Minimalformia:
    ...
    def __init__(self, config: MinimalformiaConfig):
        self.config = config
    ...
    def _setup_logger(self) -> logging.Logger:
        logger = logging.getLogger(f"Minimalformia_{self.tenant_id}")
        logger.setLevel(logging.INFO)
        
        if not logger.handlers:
            handler = logging.StreamHandler()
            formatter = logging.Formatter(
                f'%(asctime)s - Minimalformia - {self.tenant_id} - %(levelname)s - %(message)s'
            )
    ...
    def _initialize_components(self):
        ...
        if self.config.enable_logging:
            self.logger.info(f"Minimalformia inicializado para tenant {self.tenant_id}")
...
def create_agent(tenant_id: str, **kwargs) -> Minimalformia:
    """Factory function para crear instancia del agente"""
    config = MinimalformiaConfig(tenant_id=tenant_id, **kwargs)
    return Minimalformia(config)


# Export para uso externo
__all__ = ['Minimalformia', 'create_agent', 'MinimalformiaConfig']