"""
LeadScoringIA v2.2 - Compatible con schemas canónicos
"""
...
# Usar schemas canónicos existentes
...
CHANNEL_SCORES = {"referral": 1.0, "search": 0.85, "landing_form": 0.70, "social": 0.55, "display": 0.40, "email": 0.75, "partner": 0.80}
...
class LeadScoringIA:
    ...
    def __init__(self, tenant_id: str, config: Optional[Dict[str, Any]] = None, enable_cache: bool = True, enable_fairness: bool = True, enable_rate_limit: bool = True):
        ...
        rate = self.config.get("rate_limit", {}).get("rate", 100)
        capacity = self.config.get("rate_limit", {}).get("capacity", 200)
    ...
    def _register_default_models(self):
        ...
        self.model_registry.register("engagement_focused", {"income": 0.25, "credit_score": 0.30, "channel": 0.20, "engagement": 0.25}, {"A": 0.75, "B": 0.55, "C": 0.35})
    ...
    async def execute(self, lead: Lead) -> LeadScoringOutput:
        ...
        try:
            ...
            if lead.tenant_id != self.tenant_id:
                raise ValueError(f"tenant_mismatch")
            ...
            if cached:
                ...
                return LeadScoringOutput(**cached, latency_ms=dt)
            
            result = self.circuit_breaker.call(self._score, lead)
            
            if self.fairness:
                channel_name = getattr(lead.attributes, "channel", "landing_form")
                self.fairness.record(result["score"], channel_name)
            ...
            logger.info("success tenant=%s lead=%s score=%.4f bucket=%s dt=%dms", self.tenant_id, result["lead_id"], result["score"], result["bucket"], dt)
            
            return LeadScoringOutput(**result, latency_ms=dt, tenant_id=self.tenant_id)
    ...
    def _cache_key_hmac(self, lead: Lead) -> str:
        key_data = f"{lead.lead_id}:{getattr(lead.attributes, 'channel', 'landing_form')}"
        h = hmac.new(self.cache_secret.encode(), key_data.encode(), hashlib.sha256)
        return h.hexdigest()[:32]

    def _normalize(self, value: float, *args, min: float = None, max: float = None,
                   min_val: float = None, max_val: float = None, **kwargs) -> float:
        """
        Normaliza un valor en [0,1], tolerando distintas firmas:
          - _normalize(v, min=..., max=...)
          - _normalize(v, min_val=..., max_val=...)
          - _normalize(v, <min>, <max>)  (posicional)
        """
        # Soporte posicional (v, min, max)
    ...
    def _engagement(self, events: Optional[List]) -> float:
        ...
        def etype(e): return str(getattr(e, "type", "other")).lower()
    ...
    def _score(self, lead: Lead) -> Dict[str, Any]:
        ...
        channel_name = getattr(attrs, "channel", "landing_form")
        engagement = self._engagement(lead.events)
        channel = float(self.channel_scores.get(channel_name, 0.5))
    ...
    def health(self) -> Dict[str, Any]:
        ...
        if self.cache: data["cache"] = self.cache.stats()
        if self.fairness: data["fairness"] = self.fairness.disparity()
        if self.rate_limiter: data["rate_limiter"] = self.rate_limiter.stats()
