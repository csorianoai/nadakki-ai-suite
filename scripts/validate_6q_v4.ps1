...
Write-Host "NADAKKI - Validacion 6 Preguntas (v4)" -ForegroundColor Cyan
Write-Host "Timestamp: $stamp" -ForegroundColor Gray
Write-Host ""
...
# Q1/Q2: Endpoints base y catalogo
Write-Host "== Q1/Q2: Health + Catalogo ==" -ForegroundColor Yellow

$health = SafeGet "$ProdBaseUrl/health"
Write-Host "  GET /health -> $($health.status)" -ForegroundColor $(if ($health.ok) { "Green" } else { "Red" })
L "## Q1/Q2 - Endpoints y Catalogo"
L "- GET /health: $($health.status)"

$catUrl = "$ProdBaseUrl/api/catalog?module=marketing" + "&" + "limit=300"
$catalog = SafeGet $catUrl
Write-Host "  GET /api/catalog -> $($catalog.status)" -ForegroundColor $(if ($catalog.ok) { "Green" } else { "Red" })
L "- GET /api/catalog?module=marketing: $($catalog.status)"
...
if ($catalog.ok -and $catalog.body) {
  try {
    ...
    L "- Total agentes en catalogo: $($agentList.Count)"
    L "- Con execute(): $($execList.Count)"

    $sample = ($catalog.body).Substring(0, [Math]::Min(300, $catalog.body.Length))
    L "- Sample (300 chars): $sample"
  }
  catch {
    Write-Host "  Error parseando catalogo" -ForegroundColor Red
    L "- Error parseando catalogo JSON"
  }
}
L ""

# Q3: Ejecucion con ID corto y largo
Write-Host ""
...
if ($execList.Count -ge 1) {
  ...
  $longId = [string]$testAgent.agent_id
  if (-not $longId) { $longId = [string]$testAgent.id }
  $shortId = $longId
  if ($longId -match "__") { $shortId = ($longId -split "__")[-1] }

  Write-Host "  Agent largo: $longId" -ForegroundColor Gray
  Write-Host "  Agent corto: $shortId" -ForegroundColor Gray
  L "- Agent ID largo: $longId"
  L "- Agent ID corto: $shortId"
  ...
  foreach ($route in $routes) {
    ...
    if ($route -eq "/agents/execute") {
      $bodyExec = "{`"agent_id`":`"$longId`",`"dry_run`":true}"
      $r = SafePost $url $bodyExec
    }
    else {
    ...
    if ($r.body) {
      $bodyClip = ($r.body).Substring(0, [Math]::Min(200, $r.body.Length))
      L "  Response: $bodyClip"
    }
  }
}
else {
  Write-Host "  No hay agentes ejecutables para probar" -ForegroundColor Red
  L "- No hay agentes ejecutables en catalogo"
}
L ""
...
L "- GET /api/v1/config (sin header): $($configBase.status)"

$configTenant = SafeGet $configUrl @{ "X-Tenant-ID" = "credicefi" }
Write-Host "  GET /api/v1/config (X-Tenant-ID: credicefi) -> $($configTenant.status)" -ForegroundColor $(if ($configTenant.ok) { "Green" } else { "Red" })
L "- GET /api/v1/config (X-Tenant-ID: credicefi): $($configTenant.status)"

if ($configBase.body) {
  $bodyClip = ($configBase.body).Substring(0, [Math]::Min(300, $configBase.body.Length))
  L "  Body: $bodyClip"
}
...
L "- GET /api/social/connections (credicefi): $($conn.status)"

$socialUrl = "$ProdBaseUrl/api/v1/social/status"
$social = SafeGet $socialUrl
Write-Host "  GET /api/v1/social/status -> $($social.status)" -ForegroundColor $(if ($social.ok) { "Green" } else { "Red" })
L "- GET /api/v1/social/status: $($social.status)"
L ""
...
if ($catObj) {
  ...
  Write-Host "  No hay catalogo para analizar" -ForegroundColor Red
  L "- No se pudo analizar shape"
}
L ""
...
if ($execList.Count -ge 3) {
  ...
  foreach ($s in $smoke3) {
    ...
    Write-Host "  Smoke $i : $aid ($cat)" -ForegroundColor Green
    L "| $i | $aid | $cat |"
    $i++
  }
}
elseif ($execList.Count -ge 1) {
  ...
  L "- Solo $($execList.Count) ejecutable(s) encontrados"
}
else {
  Write-Host "  0 ejecutables" -ForegroundColor Red
  L "- 0 ejecutables encontrados"
}
L ""

# REPOS CHECK
Write-Host ""
Write-Host "== SEGURIDAD: Repos publicos? ==" -ForegroundColor Yellow

L "## Seguridad - Repos"

$ghBack = SafeGet "https://api.github.com/repos/csorianoai/nadakki-ai-suite"
if ($ghBack.status -eq 404) {
  Write-Host "  Backend repo: PRIVADO (404)" -ForegroundColor Green
  L "- Backend repo: PRIVADO (404 = no accesible publicamente)"
}
elseif ($ghBack.ok) {
  Write-Host "  Backend repo: PUBLICO" -ForegroundColor Red
  L "- Backend repo: PUBLICO -- PELIGRO"
}
else {
  Write-Host "  Backend repo: status $($ghBack.status)" -ForegroundColor Yellow
  L "- Backend repo: HTTP $($ghBack.status)"
}

$ghFront = SafeGet "https://api.github.com/repos/csorianoai/nadakki-dashboard"
if ($ghFront.status -eq 404) {
  Write-Host "  Frontend repo: PRIVADO (404)" -ForegroundColor Green
  L "- Frontend repo: PRIVADO (404 = no accesible publicamente)"
}
elseif ($ghFront.ok) {
  Write-Host "  Frontend repo: PUBLICO" -ForegroundColor Red
  L "- Frontend repo: PUBLICO -- PELIGRO"
}
else {
  Write-Host "  Frontend repo: status $($ghFront.status)" -ForegroundColor Yellow
  L "- Frontend repo: HTTP $($ghFront.status)"
}
L ""
...
Write-Host "  RESUMEN FASE 0" -ForegroundColor Cyan
Write-Host "========================================" -ForegroundColor Cyan
...
$checks = @(
  ...
  @{ name = "Agentes ejecutables > 0"; pass = ($execList.Count -gt 0) },
  @{ name = "/api/v1/config 200"; pass = $configBase.ok },
  @{ name = "/api/social/connections 200"; pass = $conn.ok },
  @{ name = "Backend repo privado"; pass = ($ghBack.status -eq 404) },
  @{ name = "Frontend repo privado"; pass = ($ghFront.status -eq 404) }
)
...
foreach ($c in $checks) {
  ...
  $mdIcon = if ($c.pass) { "PASS" } else { "FAIL" }
  L "- $mdIcon: $($c.name)"
}
...
Write-Host "Abrir: notepad $reportPath"
