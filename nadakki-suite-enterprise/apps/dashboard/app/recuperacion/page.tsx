...
export default function recuperacionPage() {
  ...
  useEffect(() => {
    fetch("https://nadakki-ai-suite.onrender.com/api/catalog/recuperacion/agents")
      .then((res) => res.json())
  ...
  const executeAgent = async (agentId: string) => {
    ...
    setShowModal(true);
    try {
      const response = await fetch("https://nadakki-ai-suite.onrender.com/agents/recuperacion/" + agentId + "/execute", {
        method: "POST",
  ...
  return (
    <div className="p-8">
      <h1 className="text-3xl font-bold mb-6 capitalize">Recuperacion Agents</h1>
