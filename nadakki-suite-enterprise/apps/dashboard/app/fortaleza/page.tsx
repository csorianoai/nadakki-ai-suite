...
export default function fortalezaPage() {
  ...
  useEffect(() => {
    fetch("https://nadakki-ai-suite.onrender.com/api/catalog/fortaleza/agents")
      .then((res) => res.json())
  ...
  const executeAgent = async (agentId: string) => {
    ...
    setShowModal(true);
    try {
      const response = await fetch("https://nadakki-ai-suite.onrender.com/agents/fortaleza/" + agentId + "/execute", {
        method: "POST",
  ...
  return (
    <div className="p-8">
      <h1 className="text-3xl font-bold mb-6 capitalize">Fortaleza Agents</h1>
