"""
Earlywarning - Vigilancia Ecosystem
Detección deterioro crediticio 90 días antes
Generado automáticamente: 2025-08-07T13:29:04.012843
Performance: 96.3%
Complejidad: high
"""
...
class Earlywarning:
    ...
    def _setup_logger(self) -> logging.Logger:
        logger = logging.getLogger(f"Earlywarning_{self.tenant_id}")
        logger.setLevel(logging.INFO)
        
        if not logger.handlers:
            handler = logging.StreamHandler()
            formatter = logging.Formatter(
                f'%(asctime)s - Earlywarning - {self.tenant_id} - %(levelname)s - %(message)s'
            )
    ...
    def _initialize_components(self):
        ...
        if self.config.enable_logging:
            self.logger.info(f"Earlywarning inicializado para tenant {self.tenant_id}")
...
def create_agent(tenant_id: str, **kwargs) -> Earlywarning:
    """Factory function para crear instancia del agente"""
    config = EarlywarningConfig(tenant_id=tenant_id, **kwargs)
    return Earlywarning(config)


# Export para uso externo
__all__ = ['Earlywarning', 'create_agent', 'EarlywarningConfig']