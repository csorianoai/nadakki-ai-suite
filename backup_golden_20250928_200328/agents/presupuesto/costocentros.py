"""
Costocentros - Presupuesto Ecosystem
Distribución de costos por centros
Generado automáticamente: 2025-08-07T13:29:04.664941
Performance: 90.4%
Complejidad: medium
"""
...
class Costocentros:
    ...
    def __init__(self, config: CostocentrosConfig):
        self.config = config
    ...
    def _setup_logger(self) -> logging.Logger:
        logger = logging.getLogger(f"Costocentros_{self.tenant_id}")
        logger.setLevel(logging.INFO)
        
        if not logger.handlers:
            handler = logging.StreamHandler()
            formatter = logging.Formatter(
                f'%(asctime)s - Costocentros - {self.tenant_id} - %(levelname)s - %(message)s'
            )
    ...
    def _initialize_components(self):
        ...
        if self.config.enable_logging:
            self.logger.info(f"Costocentros inicializado para tenant {self.tenant_id}")
...
def create_agent(tenant_id: str, **kwargs) -> Costocentros:
    """Factory function para crear instancia del agente"""
    config = CostocentrosConfig(tenant_id=tenant_id, **kwargs)
    return Costocentros(config)


# Export para uso externo
__all__ = ['Costocentros', 'create_agent', 'CostocentrosConfig']