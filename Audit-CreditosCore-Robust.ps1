    ...
    if (Test-Path $FilePath) {
        try {
            ...
            if ($content -and $lines) {
                ...
                # Detectar clases (con manejo de null)
                $classMatches = $content | Select-String -Pattern "^class\s+(\w+)" -AllMatches
                if ($classMatches -and $classMatches.Matches) {
                    $analysis.classes = $classMatches.Matches | ForEach-Object { 
                        if ($_.Groups -and $_.Groups.Count -gt 1) { 
                            $_.Groups[1].Value 
                        }
                    } | Where-Object { $_ }
                }
                
                # Detectar funciones (con manejo de null)
                $functionMatches = $content | Select-String -Pattern "^def\s+(\w+)" -AllMatches
                if ($functionMatches -and $functionMatches.Matches) {
                    $analysis.functions = $functionMatches.Matches | ForEach-Object { 
                        if ($_.Groups -and $_.Groups.Count -gt 1) { 
                            $_.Groups[1].Value 
                        }
                    } | Where-Object { $_ }
                ...
                # Detectar imports (con manejo de null)
                $importMatches = $content | Select-String -Pattern "^(?:from\s+\S+\s+)?import\s+(.+)" -AllMatches
                if ($importMatches -and $importMatches.Matches) {
                    $analysis.imports = $importMatches.Matches | ForEach-Object { 
                        if ($_.Groups -and $_.Groups.Count -gt 1) { 
                            $_.Groups[1].Value 
                        }
                    } | Where-Object { $_ }
            ...
            Write-ColorOutput "Error analizando archivo: $FilePath - $($_.Exception.Message)" "Warning"
        }
    }
...
foreach ($dir in $expectedDirs) {
    ...
    $auditResult.directory_structure[$dir] = @{
        exists = $exists
        path = $fullPath
        file_count = if ($exists) { (Get-ChildItem $fullPath -File -Recurse -ErrorAction SilentlyContinue).Count } else { 0 }
    }
...
# Buscar todos los archivos Python (EXCLUYENDO .venv)
Write-ColorOutput "`nANALIZANDO ARCHIVOS PYTHON..." "Header"

$pythonFiles = Get-ChildItem $ProjectPath -Filter "*.py" -Recurse -File | Where-Object { 
...
Write-ColorOutput "Archivos Python encontrados (excluyendo venv): $($pythonFiles.Count)" "Info"
...
$configFiles = Get-ChildItem $ProjectPath -Include @("*.json", "*.yaml", "*.yml", "*.ini", "*.cfg", "*.toml") -Recurse -File | Where-Object { 
    $_.FullName -notmatch '\\\.venv\\' -and 
    $_.FullName -notmatch '\\venv\\' -and 
    $_.FullName -notmatch '\\node_modules\\' -and
    $_.FullName -notmatch '\\.git\\'
...
foreach ($ecosystem in $ecosystems.Keys) {
    ...
    $auditResult.agents_status.ecosystems[$ecosystem] = @{
        ...
        missing = $missing
        completion_percent = if ($ecosystemAgents.Count -gt 0) { [math]::Round(($implemented.Count / $ecosystemAgents.Count) * 100, 1) } else { 0 }
    }
}
...
Write-ColorOutput "`nARCHIVOS PRINCIPALES ENCONTRADOS:" "Header"
$mainFiles = $auditResult.files_analysis | Where-Object { $_.agents_detected.Count -gt 0 -or $_.classes.Count -gt 0 } | Select-Object -First 10
foreach ($file in $mainFiles) {
    $fileName = [System.IO.Path]::GetFileName($file.path)
    Write-ColorOutput "- $fileName (Lineas: $($file.lines_of_code), Clases: $($file.classes.Count), Agentes: $($file.agents_detected.Count))" "Info"
}
