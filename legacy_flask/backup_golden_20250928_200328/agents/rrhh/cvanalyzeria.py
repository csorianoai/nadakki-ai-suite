"""
Cvanalyzeria - Rrhh Ecosystem
Análisis automático de CV con ranking
Generado automáticamente: 2025-08-07T13:29:04.686750
Performance: 94.3%
Complejidad: high
"""
...
class Cvanalyzeria:
    ...
    def __init__(self, config: CvanalyzeriaConfig):
        self.config = config
    ...
    def _setup_logger(self) -> logging.Logger:
        logger = logging.getLogger(f"Cvanalyzeria_{self.tenant_id}")
        logger.setLevel(logging.INFO)
        
        if not logger.handlers:
            handler = logging.StreamHandler()
            formatter = logging.Formatter(
                f'%(asctime)s - Cvanalyzeria - {self.tenant_id} - %(levelname)s - %(message)s'
            )
    ...
    def _initialize_components(self):
        ...
        if self.config.enable_logging:
            self.logger.info(f"Cvanalyzeria inicializado para tenant {self.tenant_id}")
...
def create_agent(tenant_id: str, **kwargs) -> Cvanalyzeria:
    """Factory function para crear instancia del agente"""
    config = CvanalyzeriaConfig(tenant_id=tenant_id, **kwargs)
    return Cvanalyzeria(config)


# Export para uso externo
__all__ = ['Cvanalyzeria', 'create_agent', 'CvanalyzeriaConfig']