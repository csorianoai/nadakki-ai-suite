"""
Upsellengine - Ventascrm Ecosystem
Motor de upselling inteligente
Generado automáticamente: 2025-08-07T13:29:04.749464
Performance: 90.7%
Complejidad: medium
"""
...
class Upsellengine:
    ...
    def __init__(self, config: UpsellengineConfig):
        self.config = config
    ...
    def _setup_logger(self) -> logging.Logger:
        logger = logging.getLogger(f"Upsellengine_{self.tenant_id}")
        logger.setLevel(logging.INFO)
        
        if not logger.handlers:
            handler = logging.StreamHandler()
            formatter = logging.Formatter(
                f'%(asctime)s - Upsellengine - {self.tenant_id} - %(levelname)s - %(message)s'
            )
    ...
    def _initialize_components(self):
        ...
        if self.config.enable_logging:
            self.logger.info(f"Upsellengine inicializado para tenant {self.tenant_id}")
...
def create_agent(tenant_id: str, **kwargs) -> Upsellengine:
    """Factory function para crear instancia del agente"""
    config = UpsellengineConfig(tenant_id=tenant_id, **kwargs)
    return Upsellengine(config)


# Export para uso externo
__all__ = ['Upsellengine', 'create_agent', 'UpsellengineConfig']