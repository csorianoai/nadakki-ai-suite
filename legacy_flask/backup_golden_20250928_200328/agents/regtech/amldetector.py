"""
Amldetector - Regtech Ecosystem
Detección de lavado de dinero en tiempo real
Generado automáticamente: 2025-08-07T13:29:04.972964
Performance: 96.8%
...
class Amldetector:
    ...
    def __init__(self, config: AmldetectorConfig):
        self.config = config
    ...
    def _setup_logger(self) -> logging.Logger:
        logger = logging.getLogger(f"Amldetector_{self.tenant_id}")
        logger.setLevel(logging.INFO)
        
        if not logger.handlers:
            handler = logging.StreamHandler()
            formatter = logging.Formatter(
                f'%(asctime)s - Amldetector - {self.tenant_id} - %(levelname)s - %(message)s'
            )
    ...
    def _initialize_critical_components(self):
        ...
        if self.config.enable_logging:
            self.logger.info(f"Amldetector inicializado para tenant {self.tenant_id}")
...
def create_agent(tenant_id: str, **kwargs) -> Amldetector:
    """Factory function para crear instancia del agente"""
    config = AmldetectorConfig(tenant_id=tenant_id, **kwargs)
    return Amldetector(config)


# Export para uso externo
__all__ = ['Amldetector', 'create_agent', 'AmldetectorConfig']