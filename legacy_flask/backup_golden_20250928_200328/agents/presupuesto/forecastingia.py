"""
Forecastingia - Presupuesto Ecosystem
Forecasting financiero con machine learning
Generado automáticamente: 2025-08-07T13:29:04.658946
Performance: 88.9%
Complejidad: high
"""
...
class Forecastingia:
    ...
    def __init__(self, config: ForecastingiaConfig):
        self.config = config
    ...
    def _setup_logger(self) -> logging.Logger:
        logger = logging.getLogger(f"Forecastingia_{self.tenant_id}")
        logger.setLevel(logging.INFO)
        
        if not logger.handlers:
            handler = logging.StreamHandler()
            formatter = logging.Formatter(
                f'%(asctime)s - Forecastingia - {self.tenant_id} - %(levelname)s - %(message)s'
            )
    ...
    def _initialize_components(self):
        ...
        if self.config.enable_logging:
            self.logger.info(f"Forecastingia inicializado para tenant {self.tenant_id}")
...
def create_agent(tenant_id: str, **kwargs) -> Forecastingia:
    """Factory function para crear instancia del agente"""
    config = ForecastingiaConfig(tenant_id=tenant_id, **kwargs)
    return Forecastingia(config)


# Export para uso externo
__all__ = ['Forecastingia', 'create_agent', 'ForecastingiaConfig']