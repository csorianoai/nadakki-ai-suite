"""
Controlgastos - Contabilidad Ecosystem
Control de gastos con categorización IA
Generado automáticamente: 2025-08-07T13:29:04.569201
Performance: 92.3%
Complejidad: medium
"""
...
@dataclass
class ControlgastosConfig:
    tenant_id: str
    performance_threshold: float = 92.3
...
class Controlgastos:
    ...
    def __init__(self, config: ControlgastosConfig):
        self.config = config
        self.tenant_id = config.tenant_id
        self.performance_score = 92.3
    ...
    def _setup_logger(self) -> logging.Logger:
        logger = logging.getLogger(f"Controlgastos_{self.tenant_id}")
        logger.setLevel(logging.INFO)
        
        if not logger.handlers:
            handler = logging.StreamHandler()
            formatter = logging.Formatter(
                f'%(asctime)s - Controlgastos - {self.tenant_id} - %(levelname)s - %(message)s'
            )
    ...
    def _initialize_components(self):
        ...
        if self.config.enable_logging:
            self.logger.info(f"Controlgastos inicializado para tenant {self.tenant_id}")
...
def create_agent(tenant_id: str, **kwargs) -> Controlgastos:
    """Factory function para crear instancia del agente"""
    config = ControlgastosConfig(tenant_id=tenant_id, **kwargs)
    return Controlgastos(config)


# Export para uso externo
__all__ = ['Controlgastos', 'create_agent', 'ControlgastosConfig']