...
# 🔁 Importar coordinador de agentes dinámicos
import coordinator
...
if COMPONENTS_LOADED:
    ...
    total_agents = sum(len(agents) for agents in app.agent_orchestrator.agent_registry.values())
    print("✅ Todos los componentes cargados exitosamente")
    print(f"🤖 {total_agents} agentes especializados registrados")
else:
...
def home():
    return jsonify({
        ...
        "version": "2.1.0",
        "description": "Sistema de Evaluación Crediticia con agentes especializados",
        "status": "operational",
        "components_loaded": COMPONENTS_LOADED,
        "total_agents": sum(len(agents) for agents in app.agent_orchestrator.agent_registry.values()) if COMPONENTS_LOADED else 0,
...
def health_check():
    return jsonify({
        ...
        "total_agents_registered": sum(len(agents) for agents in app.agent_orchestrator.agent_registry.values()) if COMPONENTS_LOADED else 0
    })
...
def evaluate_profile_full():
    """Evaluación completa usando todos los agentes"""
    try:
        profile_data = request.get_json()
        tenant_id = g.get("tenant_id", "demo")
...
if __name__ == "__main__":
    ...
    if COMPONENTS_LOADED:
        total_agents = sum(len(agents) for agents in app.agent_orchestrator.agent_registry.values())
        print(f"🎯 {total_agents} Specialized Agents Registered")
...
try:
    ...
    if success:
        print("Collections Blueprint registrado exitosamente")
        ...
        print("Error registrando Collections Blueprint")
        
except ImportError as e:
    print(f"Collections module not available: {e}")
except Exception as e:
    print(f"Collections integration error: {e}")
# ============================================================================

# ============================================================================  
# COLLECTIONS INTEGRATION - Nadakki Enterprise Extension
# ============================================================================
try:
    from apps.collections.api.blueprint import collections_bp, register_collections_blueprint
...
# ============================================================================
# COLLECTIONS INTEGRATION - SINGLE INSTANCE
# ============================================================================
try:
    from apps.collections.api.blueprint import register_collections_blueprint
    
    # Registrar Collections Blueprint
    collections_registered = register_collections_blueprint(app)
    
    if collections_registered:
        @app.route('/api/v1/collections-status')
    ...
    app.logger.error(f"Collections integration failed: {e}")
# ============================================================================
