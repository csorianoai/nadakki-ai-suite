"""
Processgenius - Operacional Ecosystem
Optimización de procesos automática con IA
Generado automáticamente: 2025-08-07T13:29:04.078077
Performance: 91.8%
Complejidad: medium
"""
...
class Processgenius:
    ...
    def __init__(self, config: ProcessgeniusConfig):
        self.config = config
    ...
    def _setup_logger(self) -> logging.Logger:
        logger = logging.getLogger(f"Processgenius_{self.tenant_id}")
        logger.setLevel(logging.INFO)
        
        if not logger.handlers:
            handler = logging.StreamHandler()
            formatter = logging.Formatter(
                f'%(asctime)s - Processgenius - {self.tenant_id} - %(levelname)s - %(message)s'
            )
    ...
    def _initialize_components(self):
        ...
        if self.config.enable_logging:
            self.logger.info(f"Processgenius inicializado para tenant {self.tenant_id}")
...
def create_agent(tenant_id: str, **kwargs) -> Processgenius:
    """Factory function para crear instancia del agente"""
    config = ProcessgeniusConfig(tenant_id=tenant_id, **kwargs)
    return Processgenius(config)


# Export para uso externo
__all__ = ['Processgenius', 'create_agent', 'ProcessgeniusConfig']