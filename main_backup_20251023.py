...
# Agregar después de los imports, antes de las clases

def create_input_object(agent_id: str, data: Dict[str, Any]) -> Any:
    """
    Convierte dict a objeto Pydantic según el agente
    ...
    Returns:
        Objeto Pydantic apropiado o dict si no hay schema
    """
    # Import schemas dinámicamente
    try:
        from schemas.canonical import (
            Lead, Campaign, Customer, Touchpoint,
            Content, Creative, Competitor, Journey
        )
        
        # Mapeo de agentes a sus schemas
        schema_mapping = {
            "leadscoringia": Lead,
            "lead_scoring": Lead,
            "customersegmentation": Customer,
            "customer_segmentation": Customer,
            "campaignoptimizer": Campaign,
            "campaign_optimizer": Campaign,
            # Agregar más según necesidad
        }
        
        # Si hay un schema específico, usarlo
        if agent_id in schema_mapping:
            schema_class = schema_mapping[agent_id]
            try:
                return schema_class(**data)
            except Exception as e:
                logger.warning(f"No se pudo crear objeto {schema_class.__name__}: {e}")
                return data
        
        # Si no hay schema, devolver dict
        return data
        
    except ImportError:
        # Si no hay schemas, devolver dict
        return data
