"""
Logisticareversa - Logistica Ecosystem
Gestión de logística reversa
Generado automáticamente: 2025-08-07T13:29:04.837381
Performance: 87.9%
Complejidad: medium
"""
...
class Logisticareversa:
    ...
    def __init__(self, config: LogisticareversaConfig):
        self.config = config
    ...
    def _setup_logger(self) -> logging.Logger:
        logger = logging.getLogger(f"Logisticareversa_{self.tenant_id}")
        logger.setLevel(logging.INFO)
        
        if not logger.handlers:
            handler = logging.StreamHandler()
            formatter = logging.Formatter(
                f'%(asctime)s - Logisticareversa - {self.tenant_id} - %(levelname)s - %(message)s'
            )
    ...
    def _initialize_components(self):
        ...
        if self.config.enable_logging:
            self.logger.info(f"Logisticareversa inicializado para tenant {self.tenant_id}")
...
def create_agent(tenant_id: str, **kwargs) -> Logisticareversa:
    """Factory function para crear instancia del agente"""
    config = LogisticareversaConfig(tenant_id=tenant_id, **kwargs)
    return Logisticareversa(config)


# Export para uso externo
__all__ = ['Logisticareversa', 'create_agent', 'LogisticareversaConfig']